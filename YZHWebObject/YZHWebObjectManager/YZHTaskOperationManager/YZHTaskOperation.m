...
#define SET_PROPERTY(PROPERTY,...)  [self willChangeValueForKey:PROPERTY]; \
                                    __VA_ARGS__; \
...
-(void)setFinished:(BOOL)finished
{
    @synchronized (self) {
        if (_finished != finished) {
            SET_PROPERTY(@"isFinished", _finished = finished)
        }
    }
}
...
-(void)setExecuting:(BOOL)executing
{
    @synchronized (self) {
        if (_executing != executing) {
            SET_PROPERTY(@"isExecuting",_executing = executing)
        }
    }
}
